...
@:genericBuild(jsonStream.rpc.Future.FutureBuilder.build())
...
class FutureBuilder
{
  macro public static function build():ComplexType return
  {
    ...
    {
      case TInst(_.get() => { name: "Future", module: "jsonStream.rpc.Future", }, [ parameterType ]):
      {
        if (parameterType.match(TAbstract(_.get() => { name: "Void", pack: [], }, [])))
        {
          macro : jsonStream.rpc.Future.Future0;
        }
        else
        {
          var parameterComplexType = TypeTools.toComplexType(parameterType);
          macro : jsonStream.rpc.Future.Future1<$parameterComplexType>;
        }
      }
      default:
      {
